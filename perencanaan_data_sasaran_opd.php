...
if(isset($_SESSION["USER_SETTING"]["LOGIN"])){
	...
	$sql1="SELECT * FROM sasaran_perangkat_daerah where sub_opd_id='".$temp_session["SESSION"]["SUB_OPD_ID"]."'";
	// if (substr($prgfull,4,2)=="01"){
	// 	$sql1="SELECT * FROM master_indikator WHERE urs_id =00  AND bid_urs_id =00";
	...
	if ($numrows > 0) {
		...
		for ($i=0; $i<$numrows; $i++) {
			...
			$data["NO"]					    			= $i + 1;
			$data["SASARAN_ROW_ID"]						= $row->row_id;
			$data["SASARAN_KODE_OPD"]					= $row->opd_id;
			$data["SASARAN_KODE_SUBOPD"]				= $row->sub_opd_id;	
			$data["SASARAN_DESKRIPSI"]					= $row->sasaran_pd;	
			$data_table[] = $data;
		}
