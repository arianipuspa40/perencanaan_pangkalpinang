...
if(isset($_SESSION["USER_SETTING"]["LOGIN"])){
	...
	$kegfull	= $_GET['kegfull'];
	// $param1 	= json_decode($_POST['param1']);

	$sql1="";
	if (substr($kegfull,4,2)=="01"){
		$sql1="SELECT * FROM master_program WHERE CONCAT(urs_id, bid_urs_id,prog_id,keg_kode,keg_id) = '"."000001".substr($kegfull,6,3)."'  AND lvl = 5 ";
		...
		$sql1="SELECT * FROM master_program WHERE CONCAT(urs_id, bid_urs_id,prog_id,keg_kode,keg_id) = '".$kegfull."'  AND lvl = 5";
