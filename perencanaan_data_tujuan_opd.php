...
if(isset($_SESSION["USER_SETTING"]["LOGIN"])){
	...
	$sql1="SELECT * FROM tujuan_perangkat_daerah where sub_opd_id='".$temp_session["SESSION"]["SUB_OPD_ID"]."'";
	...
	if ($numrows > 0) {
		...
		for ($i=0; $i<$numrows; $i++) {
			...
			$data["NO"]					    			= $i + 1;
			$data["TUJUAN_ROW_ID"]						= $row->row_id;
			$data["TUJUAN_KODE_OPD"]					= $row->opd_id;
			$data["TUJUAN_KODE_SUBOPD"]					= $row->sub_opd_id;	
			$data["TUJUAN_DESKRIPSI"]					= $row->tujuan_pd;	
			$data_table[] = $data;
		}
