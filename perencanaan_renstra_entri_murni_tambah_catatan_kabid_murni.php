	...
	if ($lvl == "program"){
		if ($action == "add"){		
			...
			if (strlen($prgfull) == 0 or strlen($sub_opd_id) == 0){
				$tes["status"] = "PROGRAM HARUS DIISI";
			...
			$sql = "UPDATE program_jangka_menengah SET catatan_reject='".$catatan."' WHERE row_id=".$rowid;
				// // $tes["sqltes"]  = $sql;
			$result1		= bmysqli_query($conn,$sql);			// True/Resource on success, False on error
		...
		if ($action == "add"){	
			...
			// 	// $tes["status"]  = $sql3;
				$result3		= bmysqli_query($conn,$sql3);			// True/Resource on success, False on error
			...
			$num_result2 	= mysql_affected_rows();	// Returns the number of affected rows on success, and -1 if the last query failed
