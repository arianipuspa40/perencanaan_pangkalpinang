...
// $prgfull	= isset($_GET['prgfull']);

$sql1="";


if (strlen($param_opd) == 11){	
	$sql1="SELECT * FROM master_program WHERE (CONCAT(urs_id, bid_urs_id) = 000 OR CONCAT(urs_id, bid_urs_id) = ".substr($param_opd,0,3)." OR CONCAT(urs_id, bid_urs_id) = ".substr($param_opd,3,3)." OR CONCAT(urs_id, bid_urs_id) = ".substr($param_opd,6,3).") AND lvl = 4";
}else{
	$sql1="SELECT * FROM master_program WHERE lvl = 4";
}
