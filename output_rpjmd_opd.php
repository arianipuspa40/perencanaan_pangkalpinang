...
// $ukuran_kertas = $_GET['param1'];
// $ukuran_kertas2 = "A2-L";
// echo json_encode($ukuran_kertas2);
// $tes["status"] = $ukuran_kertas2;
// $tes["status"] = $ukuran_kertas1;
// echo json_encode($tes);


$sql = "SELECT * FROM program_jangka_menengah WHERE opd_id='" . $temp_session["SESSION"]["OPD_ID"] . "' ORDER BY sub_opd_id, prg_full";
...
$html .= '
		...
		<td rowspan="3" width="15%"><h3>Indikator Kinerja Program (outcome)</h3></td>
		<td rowspan="3" width="5%"><h3>Kondisi Kinerja  Awal RPJMD</h3></td>
		<td colspan="10" ><h3>Target </h3></td>
		<td colspan="2" rowspan="2" width="8"><h3>Kondisi Kinerja pada akhir periode RPJMD</h3></td>
		<td rowspan="3" width="10%"><h3>Perangkat Daerah Penanggung Jawab</h3></td>
...
function getUrsDesc($kode)
{
	...
	$sql_urs  = "SELECT * FROM indikator_program_jangka_menengah order by sub_opd_id, prog_id";
...
for ($i = 0; $i < $numrows; $i++) {
	...
	if ($numrows_indi_prog > 0) {
		...
		$html .= ' 
				...
				<td align="right">' . number_format($row->target_pagu_prog_tahun5, 0, ',', '.') . '</td>
				<td align="right"></td>	
				<td align="right" width="10%">' . number_format($row->target_akhir_pagu_program, 0, ',', '.') . '</td>
				<td align="justify">' . $des_subopd . '</td>
				</tr>';
	}
}
...
$sql_total  = "";
$sql_total  = "SELECT SUM(target_pagu_prog_tahun1) AS thn1, SUM(target_pagu_prog_tahun2) AS thn2, SUM(target_pagu_prog_tahun3) AS thn3, SUM(target_pagu_prog_tahun4) AS thn4, SUM(target_pagu_prog_tahun5) AS thn5, SUM(target_akhir_pagu_program) AS thn_akhir FROM program_jangka_menengah WHERE opd_id='" . $temp_session["SESSION"]["OPD_ID"] . "'";
...
if ($ukuran_kertas1 == "A2-L" || $ukuran_kertas1 == "A3-L" || $ukuran_kertas1 == "A4-L") {
	...
	header("location: output_rpjmd_opd_kertas.php");
	// echo 'tutup ';
}
