...
$sql = "SELECT * FROM program_jangka_menengah WHERE opd_id='" . $temp_session["SESSION"]["OPD_ID"] . "' ORDER BY urs_id, bid_urs_id, sub_opd_id, prog_id";
...
<tr>
    ...
    <td rowspan="3" width="12%"><h3>Bidang Urusan Pemerintahan dan Program Prioritas Pembangunan</h3></td>
    <td rowspan="3" width="10%"><h3>Indikator Kinerja Program (outcome)</h3></td>
    <td rowspan="3" width="5%"><h3>Kondisi Kinerja  Awal RPJMD</h3></td>
    <td colspan="10" width="42%"><h3>Target</h3></td>
    <td colspan="2" rowspan="2" width="25%"><h3>Kondisi Kinerja pada akhir periode RPJMD</h3></td>
    <td rowspan="3" width="8%"><h3>Perangkat Daerah Penanggung Jawab</h3></td>
...
for ($i = 0; $i < $numrows; $i++) {
	...
	if ($lvl_opd != $row->opd_id) {
		...
		// get deskripsi opd
		$sql_opd	= "";
		$sql_opd	= "SELECT * FROM opd where opd_id='" . $row->opd_id . "' ORDER BY sub_opd_id";
		$result_opd = bmysqli_query($conn, $sql_opd);
		$row_opd 	= bmysqli_fetch_object($result_opd);

		// get deskripsi sub opd
		$sql_subopd	= "";
		$sql_subopd	= "SELECT * FROM opd where sub_opd_id='" . $row->sub_opd_id . "'";
		$result_subopd = bmysqli_query($conn, $sql_subopd);
		$row_subopd 	= bmysqli_fetch_object($result_subopd);

		// get total lvl opd
		$sql_opd2	 = "";
		$sql_opd2	 = "SELECT SUM(target_pagu_prog_tahun1) AS thn1, SUM(target_pagu_prog_tahun2) AS thn2, SUM(target_pagu_prog_tahun3) AS thn3, SUM(target_pagu_prog_tahun4) AS thn4, SUM(target_pagu_prog_tahun5) AS thn5, SUM(target_akhir_pagu_program) AS thn_akhir FROM program_jangka_menengah WHERE  opd_id='" . $row->opd_id . "'";
		$result_opd2 = bmysqli_query($conn, $sql_opd2);
		$row_opd2 	 = bmysqli_fetch_object($result_opd2);

		// set deskripsi opd
		$des_opd = $row_opd->deskripsi_opd;
		$des_subopd = $row_subopd->deskripsi_opd;
		$html .= '
			<tr>
			<td align="justify" style="font-weight:bold"></td>
			<td align="justify" colspan="3" style="font-weight:bold">' . strtoupper($des_opd) . '</td>
	...
	if ($lvl_urs != $row->urs_id) {
		...
		$sql_urs2	= "";
		$sql_urs2	= "SELECT SUM(target_pagu_prog_tahun1) AS thn1, SUM(target_pagu_prog_tahun2) AS thn2, SUM(target_pagu_prog_tahun3) AS thn3, SUM(target_pagu_prog_tahun4) AS thn4, SUM(target_pagu_prog_tahun5) AS thn5, SUM(target_akhir_pagu_program) AS thn_akhir FROM program_jangka_menengah WHERE urs_id='" . $row->urs_id . "'" . " AND opd_id='" . $temp_session["SESSION"]["OPD_ID"] . "'";
	...
	if ($lvl_urs_bidang != $row->bid_urs_id) {
		...
		$sql_bid2	 = "";
		$sql_bid2	 = "SELECT SUM(target_pagu_prog_tahun1) AS thn1, SUM(target_pagu_prog_tahun2) AS thn2, SUM(target_pagu_prog_tahun3) AS thn3, SUM(target_pagu_prog_tahun4) AS thn4, SUM(target_pagu_prog_tahun5) AS thn5, SUM(target_akhir_pagu_program) AS thn_akhir FROM program_jangka_menengah WHERE urs_id='" . $row->urs_id . "' AND bid_urs_id='" . $row->bid_urs_id . "'" . " AND opd_id='" . $temp_session["SESSION"]["OPD_ID"] . "'";
		...
		for ($i2 = 0; $i2 < $numrows_indi_prog; $i2++) {
			...
			if ($i2 == 0) {
				$html .= ' 
					...
					<td align="justify">' . $des_subopd . '</td>
					</tr>';
			} else {
				$html .= ' 
					<tr>					
...
// get total lvl urusan
$sql_total	= "";
$sql_total	= "SELECT SUM(target_pagu_prog_tahun1) AS thn1, SUM(target_pagu_prog_tahun2) AS thn2, SUM(target_pagu_prog_tahun3) AS thn3, SUM(target_pagu_prog_tahun4) AS thn4, SUM(target_pagu_prog_tahun5) AS thn5, SUM(target_akhir_pagu_program) AS thn_akhir FROM program_jangka_menengah WHERE  opd_id='" . $row->opd_id . "'";
...
if ($ukuran_kertas1 == "A2-L" || $ukuran_kertas1 == "A3-L" || $ukuran_kertas1 == "A4-L") {
	...
	header("location: output_rpjmd_opd_urs_kertas.php");
	// echo 'tutup ';
}
